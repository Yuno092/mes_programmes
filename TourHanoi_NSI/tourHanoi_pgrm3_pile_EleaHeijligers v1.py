...
# Permet de connaitre la Pile dans la tour qui contient le plus grand element
def tourPGElement (Tour):
    if Tour[0].est_vide() == False:
        valTour0 = int(Tour[0].lire_Sommet())
    else :
        valTour0 = 0
...
def initTour(nbDisques, nom) :
    tige = Pile(nom)
    for i in range (1, nbDisques+1):
        tige.empiler(str(i))
    return tige 
...
resultatAttendu = Pile ("resultatAttendu")
resultatAttendu.empiler("5")
resultatAttendu.empiler("4")
resultatAttendu.empiler("3")
resultatAttendu.empiler("2")
resultatAttendu.empiler("1")
